...
The correlation between mass and elite tolerance scores is 0.52; between
mass tolerance scores and repression scores, −0.26; between elite tolerance
scores and repression scores,−0.42.
...
The formula for the standard error of the coefficients is $SE_{\hat\beta}=\hat \sigma^{2}[X'X]^{-1}$. 
...
Given that the regression is standardized, the variance of the coefficients are equal to 1, so the sample size cancels out when we compute each matrix, as shown below.
...
$\hat \beta = (M'M)^{-1}M'U$ = 
n$\begin{pmatrix} 1 & r_{VU} \\ r_{XU} \end{pmatrix} * \frac{1}{n\begin{pmatrix} 1 & r_{VX} \\ r_{VX} & 1 \end{pmatrix}}$
